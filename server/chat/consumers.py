...
"""
    This consumer can be used for workspace activity chat. It is a sync consumer.
    This socket is not currently used in the project. It was added just to get
    familiar with sync consumers.
"""
...
class WorkspaceActivityAsyncConsumer(AsyncJsonWebsocketConsumer):
    """ 
    This consumer is used to handle the messages sent to the workspace activity chat.
    It is a async consumer. If the request's user does not belongs to the workspace room
    ...
    async def connect(self):
        ...
        if user.is_authenticated and workspace:
            try:
                is_member, workspace_room = await is_workspace_room_member(user, workspace)
                if is_member:

                    self.group_name = slugify(
                        f'{workspace_room.name}-{workspace_room.id}')
                    await self.channel_layer.group_add(self.group_name, self.channel_name)
                else:
                    await self.send_json("You are not member of this workspace room")
                    await self.close(code=4000)
            except:
                await self.send_json("Invalid workspace id")
                await self.close(code=4000)
        else:
            await self.send_json("valid token and workspace is required in params to establish the connection!")
            await self.close(code=4000)
    ...
    async def receive_json(self, content, **kwargs):
        ...
        if user.is_authenticated:
            message_obj = await create_ws_room_msg_object(workspace, user, message, is_file, attachment_file)
...
"""
    ...
    workspace room then he is not allowed to connect and send the messages.
"""
...
class CustomSendOfferStatusChangeConsumer(AsyncJsonWebsocketConsumer):
    async def connect(self):

        await self.accept()
        user = self.scope['user']
        if user.is_authenticated:
            self.group_name = "send_offer_status"
            await self.channel_layer.group_add(self.group_name, self.channel_name)
        else:
            await self.send_json("login required")
            await self.close(code=4000)
    ...
    async def send_offer_status(self, event):
        await self.send_json(event['message'])
